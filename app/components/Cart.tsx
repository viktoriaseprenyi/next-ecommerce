...
const Cart = () => {
  ...
  return (
    <motion.div animate={{opacity:1}} initial={{opacity:0}} exit={{opacity:0}} onClick={()=> cartStore.toggleCart()} className="fixed w-full h-screen left-0 top-0 bg-black/25">
      ...
      {/*Actual Cart*/}
      <motion.div layout onClick={(e) => e.stopPropagation()} className="bg-base-200 absolute right-0 top-0 w-full lg:w-2/5 h-screen p-12 overflow-y-scroll">
     {cartStore.onCheckout === "cart" && (
      ...
      <button
        ...
        {cartStore.cart.map((item)=> (
          <motion.div layout key={item.id} className="flex p-4 gap-4 my-2 rounded-lg bg-base-100">
